 ...
 * 
 */
class com.ElTorqiro.UITweaks.Plugins.Inspecto.ItemStats {
	...
	public static function GetItemStats(inventory:Inventory, itemPosition:Number):Object {
		...
		switch( item.type ) {
			...
			case _global.Enums.ItemType.e_ItemType_Chakra:
				...
				for ( var s:String in tooltipData.m_Attributes ) {
					if ( tooltipData.m_Attributes[s].m_Right != undefined ) {
						...
						attribute = CommonUtils.stripHtml( tooltipData.m_Attributes[s].m_Right );
				...
				if ( tooltipData.m_PrefixData != undefined ) {
					item.glyph = { name: CommonUtils.stripHtml(tooltipData.m_PrefixData.m_Title), rank: Number(tooltipData.m_PrefixData.m_ItemRank) };
					...
					for ( var s:String in tooltipData.m_PrefixData.m_Attributes ) {
						if ( tooltipData.m_PrefixData.m_Attributes[s].m_Right != undefined ) {
							...
							attribute = CommonUtils.stripHtml( tooltipData.m_PrefixData.m_Attributes[s].m_Right );
							statIndex = attribute.indexOf( '+' ) + 1;
							value = attribute.substring( statIndex, attribute.indexOf(' ', statIndex));
							stat = attribute.substr( statIndex + value.length + 1 ).toLowerCase();
				...
				if ( tooltipData.m_SuffixData != undefined ) {
					...
					item.signet = { name: CommonUtils.stripHtml(tooltipData.m_SuffixData.m_Title), rank: Number(tooltipData.m_SuffixData.m_ItemRank) };
					item.signet.attributes = { };
					
					if ( tooltipData.m_SuffixData.m_Descriptions != undefined ) {
						attribute = CommonUtils.stripHtml( tooltipData.m_SuffixData.m_Descriptions[0] );
						var numbers:Array = CommonUtils.extractNumbers( attribute );
						if( numbers.length == 1 ) {
			...
			case _global.Enums.ItemType.e_ItemType_AegisWeapon:
				...
				item.attributes[ 'aegis xp percent' ] = { name: 'aegis xp percent', value: CommonUtils.extractNumbers( CommonUtils.stripHtml(tooltipData.m_Descriptions[2]) )[0] };
				
			case _global.Enums.ItemType.e_ItemType_AegisGeneric:
				
				if( item.typePosition == undefined ) item.typePosition = 'talisman';
				
				var attribute = CommonUtils.stripHtml( tooltipData.m_Descriptions[0] );
				if ( attribute != undefined ) {
					
					if ( item.name == LDB.GetText('ItemNames', 'AECapacitor') ) {
						item.attributes[ 'aegis damage' ] = { name: 'aegis damage', value: CommonUtils.extractNumbers( attribute )[0] };
					}
					
					else if ( item.name == LDB.GetText('ItemNames', 'AERampartCapacitor') ) {
						item.attributes[ 'aegis shield' ] = { name: 'aegis shield', value: CommonUtils.extractNumbers( attribute )[0] };
					}
					
					else if ( item.name == LDB.GetText('ItemNames', 'AEConvalescenceCapacitor') ) {
						item.attributes[ 'aegis healing' ] = { name: 'aegis healing', value: CommonUtils.extractNumbers( attribute )[0] };						
					}
					
					// aegis disruptor
					else {
						item.attributes[ 'aegis damage' ] = { name: 'aegis damage', value: CommonUtils.extractNumbers( attribute )[0] };
						item.attributes[ 'aegis healing' ] = { name: 'aegis healing', value: CommonUtils.extractNumbers( attribute )[1] };
					}
				}
			...
			case _global.Enums.ItemType.e_ItemType_AegisShield:
				...
				item.attributes[ 'aegis shield' ] = { name: 'aegis shield', value: CommonUtils.extractNumbers( CommonUtils.stripHtml(tooltipData.m_Descriptions[0]) )[0] };
			break;
